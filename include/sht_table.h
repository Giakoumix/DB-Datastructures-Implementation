...
// Δομη για αποθηκευση του (name, pointer->block) πιο οργανωμενα
typedef struct {
    char name[15];
    int pointer;
} Pair;
...
typedef struct {
    ...
    char file_type; 
    int file_desc;
    int blocks_per_index; // Ποσα Pairs που αντιστοιχουν σε blocks χωρανε
    int block_counter; // Ποσα blocks ειναι allocated
    int buckets;
    char *ht_name;
