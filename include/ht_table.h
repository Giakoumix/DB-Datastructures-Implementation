...
typedef int (*Hash_Func) (void *, long);
...
typedef struct {
    ...
    char file_type;
    int file_desc;
    int **ht; // Το Hash Table
    int block_counter; // Ποσα blocks εχουν γινει allocated
    int records_per_block; // Ποσα records χωρανε σε ενα block
    long int num_buckets;
    Hash_Func hash_func; // Kραταω την συναρτηση κατακερματισμου στην δομη
} HT_info;


// typedef long int (*Hash_Func) (HT_info *, long);
